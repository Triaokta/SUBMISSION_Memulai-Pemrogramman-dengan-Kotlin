...
fun getMiddleCharacters(string: String): String {
    ...
    val midChar = if (string.length % 2 == 0) {
        string.substring(midIndex - 1, midIndex + 1)
    } else {
        string.substring(midIndex, midIndex + 1)
    }
    println("panjang string \"$string\" adalah ${string.length} (${if (string.length % 2 == 0) "genap" else "ganjil"}) dan dua karakter paling tengah adalah \"$midChar\"")
    return midChar
}
