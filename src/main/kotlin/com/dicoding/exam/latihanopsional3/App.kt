...
fun manipulateString(str: String, int: Int): String {
    val regex = Regex("""\d+""")
    val numbInString: Int = regex.find(str)?.value?.toInt() ?: 0
    val result = if (numbInString != 0) {
        (numbInString * int).toString()
    } else {
        int.toString()
    }
    val output = str.replace(regex, result)
    val explanation = when {
        numbInString != 0 -> "string mengandung angka $numbInString. $numbInString*$int = ${numbInString * int} -> $output"
        else -> "string tidak mengandung angka. Langsung gabungkan $str dengan $int -> $output$int"
    }
    println(explanation)
    return output
}
